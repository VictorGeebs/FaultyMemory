...
class BinaryWeightMemory(object):
    ...
    def __init__(self,
        ...
        for m in model.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                if not skipfirst or skipfirst and not skipped == 0:
                    ...
                    if hasattr(m, 'name') and m.name == 'convShortcut' and shortcutavoid:
                        self.pis[-1] = 0.5
                    self.layer_cardinality.append(npar)
                    self.observed_fault_rate.append(0.)
                    self.mask_faulty.append(0.)
                    self.maxcons += npar
                    self.params += 1
                else:
                    skipped += 1
        ...
        # print('***' * 10)
        assert self.layer_cardinality.__len__() == self.saved_params.__len__(
        ) == self.actual_params.__len__() == self.params
    ...
    def binarize(self):
        ...
        for i in range(self.params):
            ...
            mask_faulty = torch.where(mask_faulty >= self.pis[i], torch.tensor(
    ...
    def differentiate(self, new_state):
        ...
        '''
        assert new_state.__len__() == self.pis.__len__(
