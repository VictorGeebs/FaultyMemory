...
class RepresentedTensor(ABC):
    ...
    def __str__(self) -> str:
        ...
        string += tabulate(self.pert,
                           ['Perturbation type', 'Params.'], tablefmt="github")
        string += tabulate(self.tensor_stats,
                           ['Stat.', 'Value'], tablefmt="github")
        string += tabulate(sorted([(v['priority'], k) for k, v in self.callbacks.items()]), [
                           'Priority', 'Callback'], tablefmt="github")
        return string
    ...
    def register_callback(self, callback: Callable, name: str, priority: float, autoremove: bool = False) -> None:
        self.callbacks[name] = {'priority': priority,
                                'func': callback,
                                'autoremove': autoremove}

    def access_ten_before(self, callback: Callable, autoremove: bool = False, name: str = 'before_quantize') -> None:
        self.register_callback(callback,
                               name,
                               float('-inf'),
                               autoremove)

    def access_ten_after(self, callback: Callable, autoremove: bool = False, name: str = 'after_quantize') -> None:
        self.register_callback(callback,
                               name,
                               float('inf'),
                               autoremove)
    ...
    def apply_perturb_to_encoded(self, base) -> torch.Tensor:
        for _, pert in self.pert.items():
            ...
            assert self.repr.compatibility(
                pert), 'The perturbation is not compatible with the representation'
            base = pert(base)
        return base
    ...
    def energy_consumption(self, a=12.8) -> Tuple[int, float]:
        assert 'bitcount' in self.tensor_stats.keys(
        ), 'Bitcount has not been set in `compute_bitcount`'
        if 'BernoulliXORPerturbation' in self.pert:
            p = self.pert['BernoulliXORPerturbation'].distribution.probs.cpu(
            ).numpy()
        else:
            print(
    ...
    def adjust_fixed_point(self) -> None:
        ...
        '''
        if not 'MAX' in self.tensor_stats.keys() and not 'MIN' in self.tensor_stats.keys() and not 'ValueRange' in self.callbacks and not 'DynamicValueRange' in self.callbacks:
            print(
        ...
        self.register_callback(func, name='TuneRepr',
                               priority=-1, autoremove=True)
    ...
    def compute_bitcount(self) -> None:
        def func(self, output) -> None:
            self.tensor_stats['bitcount'] = (
                output.numel() / output.shape[0]) * self.repr.width
        self.access_ten_after(func, name='compute_bitcount', autoremove=True)
