...
IMAGENET_NORMALIZE = transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
...
class DataHolder:
    ...
    def access_dataset(self) -> Tuple[Dataset, Dataset]:
        ...
        kwargs = {'root': self.path,
                  'download': True}  # TODO: test if download is necessary 
        return (base(train=True, transform=self.transform_train(), **kwargs),
    ...
    def access_dataloader(self) -> Tuple[DataLoader, DataLoader]:
        ...
        return (self._dataloader_factory(trainset), self._dataloader_factory(testset, False))
